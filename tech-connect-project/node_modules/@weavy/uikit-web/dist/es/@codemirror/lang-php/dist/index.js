import { ExternalTokenizer as S, styleTags as o, tags as $, LRParser as t, LRLanguage as l, indentNodeProp as x, continuedIndent as W, delimitedIndent as R, foldNodeProp as V, foldInside as r, html as U, LanguageSupport as v, parseMixed as u } from "../../../editor.js";
const b = 1, m = 2, p = 263, k = 3, c = 264, s = 265, Y = 266, Z = 4, w = 5, _ = 6, h = 7, X = 8, j = 9, G = 10, f = 11, g = 12, E = 13, I = 14, N = 15, F = 16, C = 17, L = 18, A = 19, H = 20, K = 21, D = 22, B = 23, M = 24, J = 25, OO = 26, $O = 27, QO = 28, iO = 29, yO = 30, aO = 31, zO = 32, SO = 33, PO = 34, WO = 35, eO = 36, TO = 37, sO = 38, XO = 39, dO = 40, nO = 41, qO = 42, oO = 43, tO = 44, lO = 45, xO = 46, RO = 47, VO = 48, rO = 49, UO = 50, vO = 51, uO = 52, bO = 53, mO = 54, pO = 55, kO = 56, cO = 57, YO = 58, ZO = 59, wO = 60, _O = 61, e = 62, hO = 63, jO = 64, GO = 65, fO = {
  abstract: Z,
  and: w,
  array: _,
  as: h,
...
const gO = {
  ...
  if (O.next == 40) {
    ...
    for (; n(O.peek(Q)); )
      Q++;
    O.peek(Q) == 41 && gO[i.toLowerCase()] && O.acceptToken(b);
    ...
    for (; O.next == 32 || O.next == 9; )
      O.advance();
    let Q = O.next == 39;
    if (Q && O.advance(), !a(O.next))
      return;
    let i = String.fromCharCode(O.next);
    for (; O.advance(), !(!a(O.next) && !(O.next >= 48 && O.next <= 55)); )
    ...
    for (; ; ) {
      ...
      if (y) {
        ...
        if (z)
          return O.acceptToken(m);
      }
    }
  }
}), IO = new S((O) => {
  O.next < 0 && O.acceptToken(Y);
}), NO = new S((O, Q) => {
  O.next == 63 && Q.canShift(s) && O.peek(1) == 62 && O.acceptToken(s);
});
function FO(O) {
  let Q = O.peek(1);
  ...
  if (Q >= 48 && Q <= 55) {
    let i = 2, y;
    for (; i < 5 && (y = O.peek(i)) >= 48 && y <= 55; )
      i++;
    return i;
  }
  if (Q == 120 && T(O.peek(2)))
    return T(O.peek(3)) ? 4 : 3;
...
const CO = new S((O, Q) => {
  let i = !1;
  for (; !(O.next == 34 || O.next < 0 || O.next == 36 && (a(O.peek(1)) || O.peek(1) == 123) || O.next == 123 && O.peek(1) == 36); i = !0) {
    if (O.next == 92) {
      let y = FO(O);
      if (y) {
        if (i)
          break;
        return O.acceptToken(k, y);
      }
    } else if (!i && (O.next == 91 || O.next == 45 && O.peek(1) == 62 && a(O.peek(2)) || O.next == 63 && O.peek(1) == 45 && O.peek(2) == 62 && a(O.peek(3))) && Q.canShift(c))
      break;
    O.advance();
  }
  i && O.acceptToken(p);
...
}), AO = { __proto__: null, static: 311, STATIC: 311, class: 333, CLASS: 333 }, HO = t.deserialize({
  version: 14,
  ...
  tokenizers: [EO, CO, NO, 0, 1, 2, 3, IO],
  topRules: { Template: [0, 72], Program: [1, 232] },
  dynamicPrecedences: { 284: 1 },
  specialized: [{ term: 81, get: (O, Q) => d(O) << 1, external: d }, { term: 81, get: (O) => AO[O] || -1 }],
  tokenPrec: 29354
}), KO = /* @__PURE__ */ l.define({
  name: "php",
  parser: /* @__PURE__ */ HO.configure({
    props: [
      /* @__PURE__ */ x.add({
        IfStatement: /* @__PURE__ */ W({ except: /^\s*({|else\b|elseif\b|endif\b)/ }),
        TryStatement: /* @__PURE__ */ W({ except: /^\s*({|catch\b|finally\b)/ }),
        SwitchBody: (O) => {
        ...
        Statement: /* @__PURE__ */ W({ except: /^({|end(for|foreach|switch|while)\b)/ })
      }),
      /* @__PURE__ */ V.add({
        "Block EnumBody DeclarationList SwitchBody ArrayExpression ValueList": r,
        ColonBlock(O) {
...
function BO(O = {}) {
  ...
  return new v(KO.configure({
    wrap: i && u((y) => y.type.isTop ? {
      parser: i.parser,
      overlay: (z) => z.name == "Text"
    } : null),

import { ExternalTokenizer as a, styleTags as o, tags as Q, LRParser as Z, LRLanguage as _, indentNodeProp as q, continuedIndent as r, foldNodeProp as l, foldInside as w, LanguageSupport as V } from "../../../editor.js";
...
const G = new a((O, i) => {
  if (n(O.next)) {
    ...
    while (e(O.next));
    if (O.next == W) {
      if (P = !0, O.advance(), n(O.next))
        do
          O.advance();
        while (e(O.next));
      else if (O.next == W || O.next > 127 || /\w/.test(String.fromCharCode(O.next)))
        return;
    }
    if (O.next == c || O.next == p) {
      if (P = !0, O.advance(), (O.next == d || O.next == f) && O.advance(), !e(O.next))
        return;
      do
        O.advance();
      while (e(O.next));
    ...
    if (O.next == v) {
      let $ = O.peek(1);
      if ($ == X + 3 && O.peek(2) == X + 2 || $ == X + 6 && O.peek(2) == X + 4)
        O.advance(3), P = !0;
      else
        return;
    }
    P && O.acceptToken(T);
  } else if (O.next == s || O.next == t) {
    if (O.next == s && O.advance(), O.next != t)
      return;
    O.advance();
    let P = 0;
    for (; O.next == Y; )
  ...
  ", ; ::": Q.separator,
  "Attribute/...": Q.meta
}), j = { __proto__: null, self: 28, super: 32, crate: 34, impl: 46, true: 72, false: 72, pub: 88, in: 92, const: 96, unsafe: 104, async: 108, move: 110, if: 114, let: 118, ref: 142, mut: 144, _: 198, else: 200, match: 204, as: 248, return: 252, await: 262, break: 270, continue: 276, while: 312, loop: 316, for: 320, macro_rules: 327, mod: 334, extern: 342, struct: 346, where: 364, union: 379, enum: 382, type: 390, default: 395, fn: 396, trait: 412, use: 420, static: 438, dyn: 476 }, E = Z.deserialize({
  version: 14,
  ...
  tokenizers: [m, u, G, 0, 1, 2, 3],
  topRules: { SourceFile: [0, 8] },
  specialized: [{ term: 281, get: (O) => j[O] || -1 }],
  tokenPrec: 15596
}), I = /* @__PURE__ */ _.define({
  name: "rust",
  parser: /* @__PURE__ */ E.configure({
    props: [
      /* @__PURE__ */ q.add({
        IfExpression: /* @__PURE__ */ r({ except: /^\s*({|else\b)/ }),
        "String BlockComment": () => null,
        AttributeItem: (O) => O.continue(),
        "Statement MatchArm": /* @__PURE__ */ r()
      }),
      /* @__PURE__ */ l.add((O) => {
        if (/(Block|edTokens|List)$/.test(O.name))
          return w;
        if (O.name == "BlockComment")
          return (i) => ({ from: i.from + 2, to: i.to - 2 });
      })
    ]
  }),
  languageData: {

...
var q = (O, e, a) => (_(O, typeof e != "symbol" ? e + "" : e, a), a);
import { ContextTracker as L, ExternalTokenizer as k, styleTags as B, tags as P, LRParser as M, LRLanguage as E, indentNodeProp as j, delimitedIndent as D, foldNodeProp as A, foldInside as H, LanguageSupport as N } from "../../../editor.js";
...
const XO = new L({
  start: s.top,
  reduce(O, e) {
    return O.type == d && (e == oO || e == lO) ? O.parent : O;
  },
  shift(O, e, a, r) {
    if (e == y)
...
const dO = new k((O, e) => {
  ...
  if (e.context.type == d) {
    O.next == 63 && (O.advance(), l(O.next) && O.acceptToken(J));
    return;
  }
  if (O.next == 45)
    O.advance(), l(O.next) && O.acceptToken(e.context.type == T && e.context.depth == X(O, O.pos - 1) ? F : y);
  else if (O.next == 63)
    O.advance(), l(O.next) && O.acceptToken(e.context.type == g && e.context.depth == X(O, O.pos - 1) ? K : Y);
  else {
    let a = O.pos;
    for (; ; )
      if (m(O.next)) {
    ...
    for (; m(O.next); )
      O.advance();
    if (O.next == 58) {
      if (O.pos == a && e.canShift(nO))
        return;
      let r = O.peek(1);
      l(r) && O.acceptTokenTo(e.context.type == g && e.context.depth == X(O, a) ? OO : z, a);
    }
  }
}, { contextual: !0 });
function SO(O) {
...
function W(O, e) {
  return O.next == 37 ? (O.advance(), U(O.next) && O.advance(), U(O.next) && O.advance(), !0) : SO(O.next) || e && O.next == 44 ? (O.advance(), !0) : !1;
}
function Z(O) {
...
function $(O) {
  for (O.advance(); !l(O.next) && S(O.tag) != "f"; )
    O.advance();
}
function h(O, e) {
...
const kO = "iiisiiissisfissssssssssssisssiiissssssssssssssssssssssssssfsfssissssssssssssssssssssssssssfif";
function S(O) {
  return O < 33 ? "u" : O > 125 ? "s" : kO[O - 33];
}
function b(O, e) {
  let a = S(O);
  return a != "u" && !(e && a == "f");
...
function w(O, e, a, r) {
  ...
  for (; ; ) {
    ...
    if (!(Q >= 0 && (Q == 58 ? b(O.peek(n + 1), a) : Q == 35 ? O.peek(n - 1) != 32 : b(Q, a))) || !a && R <= r || R == 0 && !a && (c(O, 45, n) || c(O, 46, n)))
      break;
    if (e && S(Q) == "f")
      return !1;
    for (let v = n; v >= 0; v--)
...
const bO = new k((O, e) => {
  if (O.next == 33)
    Z(O), O.acceptToken(PO);
  else if (O.next == 38 || O.next == 42) {
    let a = O.next == 38 ? rO : QO;
    $(O), O.acceptToken(a);
  } else
    O.next == 39 || O.next == 34 ? (h(O, !1), O.acceptToken(aO)) : w(O, !1, e.context.type == d, e.context.depth) && O.acceptToken(eO);
}), gO = new k((O, e) => {
  ...
  parser: /* @__PURE__ */ mO.configure({
    props: [
      /* @__PURE__ */ j.add({
        ...
        FlowMapping: /* @__PURE__ */ D({ closing: "}" }),
        FlowSequence: /* @__PURE__ */ D({ closing: "]" })
      }),
      /* @__PURE__ */ A.add({
        "FlowMapping FlowSequence": H,
